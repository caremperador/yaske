...
@section('content')
    ...
    <div class="bg-gray-800 p-6 rounded-lg shadow-lg text-white max-w-4xl mx-auto my-10">
        ...
        <div class="max-w-7xl mx-auto">
            <div class="text-base text-white md:text-3xl mb-6">Reportes de Enlaces Caídos</div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        @auth
                            @if (auth()->user()->hasRole('admin'))
                                ...
                                {{-- aqui agregar un boton para eliminar el reporte del enlace caido de la lista --}}
                            @endif
                        @endauth
                    </div>
                    <div class="mt-4">
                        {{-- Dentro del loop de $video->enlaces --}}
                        @foreach ($video->enlaces as $enlace)
                            @if ($enlace->caido)
                                <div class="flex justify-between items-center">
                                    <p class="text-red-400">El enlace {{ $enlace->tipo }} está caído: <a
                                            href="{{ $enlace->url }}" target="_blank"
                                            class="underline">{{ $enlace->url }}</a></p>
                                    @if (auth()->user()->hasRole('admin'))
                                        <form action="{{ route('delete.enlaceCaido', $enlace->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-800 text-white rounded">
                                                Eliminar
                                            </button>
                                        </form>
                                    @endif
